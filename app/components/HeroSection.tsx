            ...
            if (isHorizontal) {
                ...
                do {
                    positionIndex = Math.floor(
                        Math.random() * strategicPositions.horizontal.length,
                    );
                } while (
                    positionIndex === lastHorizontalIndex &&
                    strategicPositions.horizontal.length > 1
                );

                lastHorizontalIndex = positionIndex;
                const baseY =
                    canvas.height *
                    strategicPositions.horizontal[positionIndex];
                const variation = (Math.random() - 0.5) * gridSize * 0.5;
                const y = Math.max(
                    gridSize,
                    Math.min(canvas.height - gridSize, baseY + variation),
                );
                ...
                do {
                    positionIndex = Math.floor(
                        Math.random() * strategicPositions.vertical.length,
                    );
                } while (
                    positionIndex === lastVerticalIndex &&
                    strategicPositions.vertical.length > 1
                );

                lastVerticalIndex = positionIndex;
                const baseX =
                    canvas.width * strategicPositions.vertical[positionIndex];
                const variation = (Math.random() - 0.5) * gridSize * 0.5;
                const x = Math.max(
                    gridSize,
                    Math.min(canvas.width - gridSize, baseX + variation),
                );
            ...
            lines.forEach((line, index) => {
                ...
                if (line.type === "horizontal") {
                    ...
                    startX = Math.max(
                        0,
                        line.x1 +
                            totalDistance * Math.max(0, line.progress - 0.3),
                    );
                    startY = line.y1;
                } else {
        ...
        <section className="relative flex items-center justify-center overflow-hidden bg-black w-full max-w-full h-[70vh] lg:h-[90vh] xl:h-screen pt-20">
            ...
            <div className="relative z-10 w-full max-w-6xl mx-auto px-4 sm:px-6 text-center">
                <div className="inline-flex items-center gap-2 sm:gap-3 mb-6 sm:mb-8 px-4 sm:px-6 py-2 sm:py-3 bg-neon-orange-bright/10 border border-neon-orange/30 rounded-full">
                    <svg
                        className="w-3 h-3 sm:w-4 sm:h-4 text-neon-orange flex-shrink-0"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                        aria-hidden="true"
                    ...
                    <span className="text-xs sm:text-sm font-semibold text-neon-orange tracking-wide">
