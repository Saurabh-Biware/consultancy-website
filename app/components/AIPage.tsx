...
export default function AIPage() {
  ...
  const [aiCapabilities, setAiCapabilities] = useState<Array<{id: number, capability: string, level: number, unlocked: boolean}>>([])  
  const [futureVision, setFutureVision] = useState<Array<{id: number, vision: string, probability: number, timeline: string}>>([])  
  ...
  useEffect(() => {
    ...
    const capabilityInterval = setInterval(() => {
      setAiCapabilities([
        {id: 1, capability: 'Pattern Recognition', level: Math.random() * 100, unlocked: true},
        {id: 2, capability: 'Strategic Reasoning', level: Math.random() * 100, unlocked: Math.random() > 0.3},
        {id: 3, capability: 'Predictive Modeling', level: Math.random() * 100, unlocked: Math.random() > 0.5},
        {id: 4, capability: 'Executive Synthesis', level: Math.random() * 100, unlocked: Math.random() > 0.7},
        {id: 5, capability: 'Autonomous Strategy', level: Math.random() * 100, unlocked: Math.random() > 0.9},
      ])
    }, 2000)
    ...
    const visionInterval = setInterval(() => {
      ...
      setFutureVision([
        {id: 1, vision: visions[Math.floor(Math.random() * visions.length)], probability: 85 + Math.random() * 10, timeline: '2025'},
        {id: 2, vision: visions[Math.floor(Math.random() * visions.length)], probability: 70 + Math.random() * 20, timeline: '2027'},
        {id: 3, vision: visions[Math.floor(Math.random() * visions.length)], probability: 60 + Math.random() * 25, timeline: '2030'},
      ])
    }, 3500)
    ...
    const animateEvolution = () => {
      ...
      particles.forEach(particle => {
        ...
        particle.evolution = 0.3 + Math.sin(frame * 0.02 + particle.phase) * 0.4
        particle.phase += 0.01
      ...
      const stages = ['AUTOMATION', 'INTELLIGENCE', 'STRATEGY', 'AUTONOMY', 'SINGULARITY']
      stages.forEach((stage, i) => {
        const x = 50 + (i * (canvas.width - 100) / (stages.length - 1))
        const isActive = i === currentEra
        
        ctx.globalAlpha = isActive ? 1 : 0.4
        ctx.fillStyle = isActive ? '#FF4500' : '#666'
        ctx.beginPath()
        ctx.arc(x, timelineY, Math.max(3, isActive ? 8 : 5), 0, Math.PI * 2)
        ctx.fill()
      ...
      particles.forEach(particle => {
        ...
        ctx.beginPath()
        ctx.arc(particle.x, particle.y, Math.max(0.1, particle.size * particle.evolution), 0, Math.PI * 2)
        ctx.fill()
  ...
  const evolutionStages = [
    {
      ...
      capabilities: ['Task Automation', 'Data Processing', 'Simple Analytics']
    },
    {
      era: 'INTELLIGENCE ERA', 
      year: '2023-2024',
      description: 'Pattern recognition and intelligent decision support',
      capabilities: ['Pattern Recognition', 'Predictive Analytics', 'Decision Support']
    },
    {
      era: 'STRATEGY ERA',
      year: '2025-2026', 
      description: 'Strategic planning and executive-level reasoning',
      capabilities: ['Strategic Planning', 'Executive Reasoning', 'Market Intelligence']
    },
    {
      era: 'AUTONOMY ERA',
      year: '2027-2029',
      description: 'Autonomous strategic execution and optimization',
      capabilities: ['Autonomous Strategy', 'Self-Optimization', 'Adaptive Learning']
    },
    {
      era: 'SINGULARITY ERA',
      year: '2030+',
      description: 'Superintelligent business transformation',
      capabilities: ['Superintelligence', 'Reality Modeling', 'Infinite Optimization']
    }
  ]
  ...
  return (
    <main className="overflow-x-hidden w-full pt-20 bg-black min-h-screen">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-lg md:text-xl text-gray-300 font-serif leading-relaxed max-w-3xl mx-auto">
              From basic automation to superintelligent strategy. Watch AI evolve from task executor to strategic architect.
            </p>
          </div>
      ...
      <section className="py-8 md:py-12 px-4 md:px-6 border-y border-neon-orange/20">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-8">
            <div className="text-xs md:text-sm text-neon-orange font-mono mb-2">CURRENT_ERA</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {currentStage.capabilities.map((capability, index) => (
              <div key={index} className="bg-gray-900/50 border border-gray-700 p-6 text-center relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="text-white font-bold font-serif text-lg mb-2">{capability}</h3>
                  <div className="text-xs text-neon-orange font-mono">ACTIVE</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-12 md:py-16 px-4 md:px-6">
        <div className="max-w-7xl mx-auto">
          <div className="text-xs md:text-sm text-neon-orange font-mono mb-6 md:mb-8">CAPABILITY_EVOLUTION // REAL_TIME</div>
          <div className="space-y-4">
            {aiCapabilities.map((capability) => (
              <div key={capability.id} className={`border p-6 transition-all duration-500 ${
                capability.unlocked ? 'border-neon-orange/40 bg-gray-900/30' : 'border-gray-700 bg-gray-900/10'
              }`}>
                <div className="flex items-center justify-between mb-4">
                  <h3 className={`font-bold font-serif text-lg ${
                    capability.unlocked ? 'text-white' : 'text-gray-500'
                  }`}>
                    {capability.capability}
                  </h3>
                  <div className={`w-4 h-4 rounded-full ${
                    capability.unlocked ? 'bg-neon-orange animate-pulse' : 'bg-gray-600'
                  }`}></div>
                </div>
                
                <div className="flex items-center gap-4">
                  <div className="flex-1">
                    <div className="text-xs text-gray-400 font-mono mb-1">EVOLUTION_LEVEL</div>
                    <div className="h-2 bg-gray-700 rounded">
                      <div 
                        className={`h-full rounded transition-all duration-1000 ${
                          capability.unlocked ? 'bg-neon-orange' : 'bg-gray-600'
                        }`}
                        style={{width: `${capability.unlocked ? capability.level : 0}%`}}
                      ></div>
                    </div>
                  </div>
                  <div className={`text-sm font-mono ${
                    capability.unlocked ? 'text-neon-orange' : 'text-gray-500'
                  }`}>
                    {capability.unlocked ? Math.round(capability.level) : 0}%
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="py-12 md:py-16 px-4 md:px-6 border-t border-neon-orange/20">
        <div className="max-w-7xl mx-auto">
          <div className="text-xs md:text-sm text-neon-orange font-mono mb-6 md:mb-8">FUTURE_VISION // PREDICTIVE</div>
          <div className="grid md:grid-cols-3 gap-6 mb-12">
            {futureVision.map((vision) => (
              <div key={vision.id} className="bg-gradient-to-b from-gray-900/50 to-black border border-neon-orange/20 p-6 relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-neon-orange/5 to-transparent opacity-50"></div>
                <div className="relative z-10">
                  <div className="text-xs text-neon-orange font-mono mb-2">{vision.timeline}</div>
                  <p className="text-white font-serif text-lg mb-4 leading-relaxed">{vision.vision}</p>
                  <div className="flex items-center justify-between">
                    <div className="text-xs text-gray-400 font-mono">PROBABILITY</div>
                    <div className="text-neon-orange font-mono font-bold">{Math.round(vision.probability)}%</div>
                  </div>
                  <div className="mt-2 h-1 bg-gray-700 rounded">
                    <div 
                      className="h-full bg-neon-orange rounded transition-all duration-1000"
                      style={{width: `${vision.probability}%`}}
                    ></div>
                  </div>
                </div>
              </div>
      ...
      <section className="py-12 md:py-16 px-4 md:px-6 border-t border-neon-orange/20">
        <div className="max-w-4xl mx-auto text-center">
          <div className="text-xs md:text-sm text-neon-orange font-mono mb-4 md:mb-6">EVOLUTION_CONSULTATION</div>
          ...
          <p className="text-gray-300 font-serif text-sm md:text-base leading-relaxed max-w-2xl mx-auto mb-8">
            Don't wait for AI to evolve around you. Partner with us to architect your organization's AI evolution strategy and stay ahead of each transformative era.
          </p>
          ...
          <div className="mb-8">
            ...
            <div className="text-xs text-neon-orange font-mono">AI_EVOLUTION_PROGRESS // CONTINUOUS</div>
          </div>
